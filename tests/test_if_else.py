...
class TestIfElse(BaseTestCase):
    ...
    def test_default(self):
        ...
        expression = self.expression.solve(command=command, df=df, local={'v': 5})
        self.assertEqual(expression, 0)

    def test_brackets_0(self):
        df, columns = self._create_df()
        command = "2 if v < 2 else (0 if v < 8 else 1) | (v == 5)"
        expression = self.expression.solve(command=command, df=df, local={'v': 1})
        self.assertEqual(expression, 2)

    def test_brackets_1(self):
        df, columns = self._create_df()
        command = "2 if v < 2 else (0 if v < 8 else 1) | (v == 5)"
        expression = self.expression.solve(command=command, df=df, local={'v': 7})
        self.assertEqual(expression, 0)
    ...
    def test_brackets_3(self):
        ...
        expression = self.expression.solve(command=command, df=df, local={'v': 10})
        self.assertEqual(expression, 1)

    def test_brackets_4(self):
        df, columns = self._create_df()
        command = "2 if v < 2 else (0 if v < 8 else 1 & (v == 10))"
        expression = self.expression.solve(command=command, df=df, local={'v': 9})
        self.assertEqual(expression, 0)
    ...
    def test_apply_inside(self):
        ...
        expression = self.expression.solve(command=command, df=df).values.tolist()
